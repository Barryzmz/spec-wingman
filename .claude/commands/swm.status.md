# swm.status — SpecWingman 工作流進度檢查
...
**建議下一步：** [具體行動，例如：執行 prompts/02-extract-requirements.prompt.md，或回答 open-questions.md 中的 Q-001]
...
3. 備註欄位說明：
   - 完成：列出主要產出（例如「8 項 FR、3 項 BR」）
   - 進行中：說明缺少什麼（例如「open-questions 有 2 個 Open」）
   - 未開始：說明「無內容」即可
