...
Before recommending a non-log step, inspect `specs/05-versions/changelog.md` and confirm that the latest completed prompt step is recorded in `Related Prompt Step`.

If completed artifacts exist for a step but the changelog does not mention that step, return `LOG_READY` and recommend `/swm.log` first. Do not advance to the next workflow step until the missing log entry has been written.
